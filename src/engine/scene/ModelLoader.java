...
public class ModelLoader {
    ...
    public static final int MAX_BONES = 1000;
    ...
    private static Model loadModel(String modelId, String modelPath, TextureCache textureCache, 
        ...
        return new Model(modelId, meshDataList, boneList, animations);
    }
    ...
    private static float[] processTangents(AIMesh aiMesh, float[] normals) {
        ...
        if(buffer == null) return new float[normals.length];
        float[] data = new float[buffer.remaining() * 3];
        int pos = 0;
        while (buffer.remaining() > 0) {
    ...
    public static List<Model.Animation> processAnimations(String animPath, List<Bone> bL){

        List<Model.Animation> animations;
        
        AIScene aiScene = aiImportFile(animPath, 0);

        if (aiScene == null) {
            throw new RuntimeException("Error loading model [modelPath: " + animPath + "]");
        }

        Node rootNode = buildNodesTree(aiScene.mRootNode(), null);
        Matrix4f globalInverseTransformation = toMatrix(aiScene.mRootNode().mTransformation()).invert();
        
        animations = processAnimations(aiScene, bL, rootNode, globalInverseTransformation);
